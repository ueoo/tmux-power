...
configure_status_rows() {
    ...
    # Rows: spacer (blank or hairline) above the bar. The bar keeps tmux's own
    # compiled status-format so the window list renders exactly as on a
    # single-row status line on every tmux version. The live array is never
    # unset here: that would redraw the plain bar for a moment on every
    # reload. Re-sourcing only rewrites values already in place.
    local default_format i
    ...
    tmux_set status 2
    # prompts and messages use row 0 (tmux's default): they show on the
    # spacer row and never hide the bar
    tmux_unset message-line
}

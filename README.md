...
| `@tmux_power_gap` | `off` (default) / `on` / `line` | spacer row between panes and the bar: blank (`on`) or a hairline (`line`); prompts and messages show on the spacer row, never over the bar |

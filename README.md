<h1 align="center">🎨 Tmux Power</h1>

<p align="center">
  <em>Yet another powerline theme for tmux.</em>
</p>
...
<p align="center">
  ...
  <a href="LICENSE"><img src="https://img.shields.io/github/license/wfxr/tmux-power?label=license" alt="License"></a>
</p>
...
### ✨ This fork (ueoo/tmux-power)

Additions on top of upstream v2, all opt-in and covered by tests:

| Option | Values | Effect |
|---|---|---|
| `@tmux_power_style` | `powerline` (default) / `text` | `text` = bare coloured text, no blocks or arrows, transparent fill |
...
| `@tmux_power_command_line` | `off` (default) / `on` | Vim-style bottom row: dim idle info (pane path, pane title) and PREFIX/COPY/ZOOM/SYNC flags; prompts and messages show there, so the spacer and the bar never move |
| `@tmux_power_command_line_left` / `_right` | format | override the command line's idle content |
...
set -g @tmux_power_command_line 'on'
set -g @plugin 'ueoo/tmux-power'
```

Also changed: status colours are set with style strings and the legacy
`status-fg`/`status-bg` options are unset on load (on tmux ≥ 3.6 a stale
`status-bg` overrides `bg=default` at render time).

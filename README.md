...
Additions on top of upstream, all opt-in:

| Option | Values | Effect |
|---|---|---|
| `@tmux_power_style` | `powerline` (default) / `text` | `text` = bare coloured text, no blocks or arrows, transparent fill, current window bold |
| `@tmux_power_bg` | colour / `default` | `default` = transparent fill while keeping powerline blocks (caps point outward) |
| `@tmux_power_gap` | `off` (default) / `on` / `line` | spacer row between panes and the bar: blank (`on`) or a dim hairline (`line`) |
| `@tmux_power_gap_line_color` | colour | hairline colour (default: gray-teal in text style, `G07` otherwise) |
| `@tmux_power_text_session_color` etc. | colour | per-element text-style palette: `_session_`, `_window_`, `_current_`, `_time_`, `_date_` |

The daily-driver setup this fork was built for:
...
Also fixed here: status styles are set via style strings (legacy `status-bg`
is unset on load — on tmux ≥ 3.6 a stale value overrides `bg=default`), and
the gap row restores the whole `status-format` array before copying so
config reloads stay idempotent.
